...
Instructions
Radhika Patel
Tells the player how to play the game/what to do.
***********************/
class Instructions extends Phaser.Scene {
  ...
  create() {
    ...
    this.underline = this.add
      .line(0, 250, this.gameGrid.colWidth * 7, 0, 0, 0, 0xffffff)
      .setOrigin(0);
    ...
    this.backButton = this.add.text(
      this.gameGrid.colWidth * 6,
      this.gameGrid.cellHeight - 50,
      "Back",
      styleText
    );
    this.backButton.setInteractive().on("pointerdown", () => this.back());
    //text epxlanations
    //instruction for the bubbles
    this.bubbleInst = this.add.text(0,0,
    ...
    //instruction for the bar
    this.barInst = this.add.text(0,0,
      "If you get 4 of them right, your oxygen tank, which is on display at the bottom, is filled and you can move up one level. However, if you tap a bubble that has the same animation as the big one, you get poisoned air and die...",
      styleText
    );
    this.gameGrid.placeIndexCell(49, this.barInst);
    //rectangle to show time bar
    this.timeBar = this.add.graphics();
    //time bar styles
    this.timeBar.fillStyle(0x2653d8, 1);
    this.timeBar.fillRect(
      this.gameGrid.colWidth * 4.5,
      this.gameGrid.cellHeight * 8,
      this.gameGrid.colWidth * 2,
      this.gameGrid.cellHeight / 2
    );
  }

  //goes back to the main menu and plays click sound
  back() {
