...
this.clickSound = this.sound.add('click');
    ...
    let styleText = {
        ...
        //bubbles animation
        for (let i = 0; i <9; i++) {
          this.bubble=this.add.image(Phaser.Math.FloatBetween(0,this.gameGrid.colWidth*7), Phaser.Math.FloatBetween(0, this.gameGrid.cellHeight*12), 'bubble');
    ...
    this.title= this.add.text(this.gameGrid.colWidth*3.5, this.gameGrid.cellHeight*1.5, 'WELCOME!',styleTextTitle);
    // this.title.setFontFamily('Fuzzy Bubbles');
    this.title.setOrigin(0.5, 0.5);
    //create button to start game
    this.startButton= this.add.text(game.config.width/2, this.gameGrid.cellHeight*6, 'Start',styleText);
    this.startButton.setOrigin(0.5, 0.5);
    this.startButton.setInteractive().on('pointerdown', () => this.start());

    //create button to let player check the instructions
    this.instructionsButton= this.add.text(game.config.width/2, this.gameGrid.cellHeight*8, 'Instructions',styleText);
    ...
    this.underline =this.add.line(0,330, this.gameGrid.colWidth*7,0, 0, 0,  0xffffff).setOrigin(0);//CHANGE LINE COLOR
