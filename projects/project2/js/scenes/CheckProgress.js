/***********************
Checking player's progress
Radhika Patel
Allows the player to verify which level they're at via the the Main menu during the game.
***********************/
class CheckProgress extends Phaser.Scene {
  ...
  create() {
    ...
    for (let i = game.finalGame.currentLvl;i <= game.finalGame.currentLvl;i++) {
      //to make the diver point at the line/level the player is at
      this.startPos -= 50;
      this.diver = this.add.image(
        this.gameGrid.colWidth * 5,
        this.gameGrid.cellHeight * (this.rows - i * 1.7) - this.startPos,
        "diver"
      );
      this.diver.setScale(0.3);
    }
    //floating animaiton on diver
    this.tweens.add({
      targets: this.diver,
    ...
    //loop to create the lines and texts
    for (let i = 6; i >= 1; i--) {
    ...
    this.textSurface = this.add.text(
      this.gameGrid.colWidth * 3,
  ...
  //goes back to the pause menu and plays the click sound fx
  back() {
